...
def vimcomplete(context,match, _buffer, lineno):
    global debugstmts
    debugstmts = []

    # START.1 Ugly hack to bypass decorators.
    # I found some problems with them
    # FIXME: find a better way
    _buffer_lines = _buffer.splitlines()
    for n, line in enumerate(_buffer_lines):
        line = line.strip()
        if not line:
            continue
        if line[0] == "@":
            _buffer_lines[n] = _buffer_lines[n].replace('@', "#")
    ...
    try:
        def complsort(x,y):
            try:
                ...
                return 0
        cmpl = Completer()
        cmpl.evalsource(_buffer, int(lineno)+1)
        all = cmpl.get_completions(context,match)
        all.sort(complsort)
        #dbg("dict: %s" % dictstr)
        return all
        #dbg("Completion dict:\n%s" % all)
    except Exception, e: # vim.error:
        dbg("VIM Error: %s" % e)
        raise
    ...
    def get_completions(self,context,match):
        ...
        try:
            ...
            for m in all:
                ...
                if m and m[0] == '_': continue # ignore special methods
                try:
...
if __name__ == '__main__':
    ...
    print vimcomplete("wx.App(", "", code, 2)
